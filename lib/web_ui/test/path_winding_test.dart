  ...
  group('Convexity', () {
    ...
    test('moveto/lineto convexity', () {
      ...
      for (LineTestCase testCase in testCases) {
        ...
        checkDirection(path, testCase.direction);
      }
    });
    ...
    test('Single moveTo origin', () {
      ...
      checkDirection(path, SPathDirection.kUnknown);
    });
    ...
    test('TriLeft', () {
      ...
      path.close();
      expect(path.convexityType, SPathConvexityType.kConvex);
      checkDirection(path, SPathDirection.kCW);
    });
    ...
    test('TriRight', () {
      ...
      path.close();
      expect(path.convexityType, SPathConvexityType.kConvex);
      checkDirection(path, SPathDirection.kCCW);
    });
    ...
    test('redundant square', () {
      ...
      checkDirection(redundantSquare, SPathDirection.kCW);
    });
...
void checkDirection(SurfacePath path, int expectedDirection) {
  if (expectedDirection == null) {
    // No expectation of direction, just return.
    return null;
  }
  expect(readDirection(path), expectedDirection);
}

int readDirection(SurfacePath path) {
  SurfacePath testPath = SurfacePath.from(path);
  return CheapComputeFirstDirection(testPath).direction;
}
